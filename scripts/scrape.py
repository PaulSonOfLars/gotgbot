...
def retrieve_api_info() -> Dict:
    ...
    for x in list(dev_rules.children):
        ...
        if curr_type and curr_name and x.name == "p" and "description" not in items[curr_type][curr_name]:
            description = x.get_text()
            items[curr_type][curr_name]["description"] = description
            if curr_type == METHODS:
...
def verify_method_parameters(items: Dict):
    ...
    for method, values in items[METHODS].items():
        ...
        if len(values.get("returns")) > 1:
            print(f"{method} has multiple return types: {values.get('return')}")
